...
import pyaudio
import logging
logging.basicConfig(filename="/home/derek/projects/monitor/mylog.log")
logging.warning('logger created')
print('yep')
...
try:
    ...
    #routes for audio stream
    @app.route('/stream/stream.m3u8')
    def stream():
        return send_file('stream/stream.m3u8', mimetype='application/x-mpegURL')

    @app.route('/<path:path>')
    def segments(path):
        return send_from_directory('', path, mimetype='video/MP2T')
    #end routes for audio stream
    ...
    def getRestartServer():
        child = pexpect.spawn('sudo systemctl restart monitor.service')
        child.expect('password')
        child.sendline('bazinga1')
        time.sleep(1)
        return "done"
